...
all_ids = np.load(Path("../data/flickr_visual_tagger_targets.npz"), allow_pickle=True)['image_targets'].item()
...
def SaveDatapointsWithMasks(dataloader, subset, datasets):
    ...
    for i, (image_fn, audio_feat, audio_name, image_name, speaker) in enumerate(tqdm(dataloader, leave=False)):
        ...
        if (audio_feat.numpy() == 0).all():
            ...
            # else:
            #     no_masks += 1 
            if image_name[0].split('.')[0] in all_ids:
                
                ouputs.append(executor.submit(
                    partial(saveADatapoint, all_ids[image_name[0].split('.')[0]], this_save_fn, image_fn[0], audio_feat, args)))
            else:
                no_masks += 1 
