...
class ImageAudioDatawithMasks(Dataset):
    ...
    def _SamplePosImage(self, im_name, im_path, id):
        ...
        if len(b) == 0:
            b = list(a.keys())
        chosen_names = np.random.choice(b, size=1)
        temp = [i for name in chosen_names for i in a[name]]
        pos_img_fn = np.random.choice(temp, size=1)
        # print(pos_img_fn)

        pos_img_name1 = '_'.join(str(Path(pos_img_fn[0]).stem).split("+")[0].split('_')[0:2])
        # if im_name == pos_img_name1 or im_path == pos_img_fn[0]: 
        ...
        # data_point = np.load(pos_img_fn[2] + ".npz")
        # english_audio_feat3 = data_point["audio_feat"]
        # english_audio_feat3, english_nframes3 = self._PadFeat(english_audio_feat3)
        # imgpath = self.image_base_path / str(data_point['image'])
        # img3 = Image.open(imgpath).convert('RGB')
        # img3 = self.resize(img3)

        # img3 = self._myRandomCrop(img3)
        # img3 = self.image_normalize(img3)

        return english_audio_feat1, english_nframes1, img1#, english_audio_feat2, english_nframes2, img2, english_audio_feat3, english_nframes3, img3

    def _SampleNegImage(self, im_name, im_path, id):
        names = []
        all_ids = list(self.neg_masks_to_images[id].keys())
        neg_id = np.random.choice(all_ids, size=3)
        ...
        temp = self.neg_masks_to_images[id][neg_id[2]].copy()
        ...
        return english_audio_feat1, english_nframes1, img1, neg_id[0], english_audio_feat2, english_nframes2, img2, neg_id[1], english_audio_feat3, english_nframes3, img3, neg_id[2], names
    ...
    def __getitem__(self, index):
        ...
        neg_english_audio_feat1, neg_english_nframes1, neg_image1, neg_id1, neg_english_audio_feat2, neg_english_nframes2, neg_image2, neg_id2, neg_english_audio_feat3, neg_english_nframes3, neg_image3, neg_id3, names = self._SampleNegImage(image_name, self.data[index], id)
        
        return {"image": image, "english_feat": english_audio_feat, "english_nframes": english_nframes, "id": str(id),
        ...
        # {"pos_audio": pos_english_audio_feat3, "pos_frames": pos_english_nframes3, "pos_image": pos_image3}
        ],
...
#         b = list(a.keys())
#         b.remove(im_name)
#         chosen_names = np.random.choice(b, size=3)
#         temp = [i for name in chosen_names for i in a[name]]
#         pos_img_fn = np.random.choice(temp, size=3)
#         # print(pos_img_fn)
